(function () {
  ...
  var Asteroid = Asteroids.Asteroid = function (options) {
    ...
    var newOptions = {
      ...
      game: options.game,
      wrappable: true,
      driftOmega: 0.1 * Math.random() * Math.pow((-1), Math.floor(Math.random() * 2))
    };
    this.craters = [];
  ...
  Asteroid.prototype.explode = function (pos) {
    if (!window.explosionSound.paused) {
      window.explosionSound.pause();
      window.explosionSound.currentTime = 0;
    }
